...
export default async function BlogPostPage({ params }: Props) {
  ...
  return (
    <article className="mx-auto max-w-5xl px-5 py-10 lg:px-8">
      ...
      <header className="mx-auto mt-10 max-w-3xl text-center"><h1 className="text-3xl font-black leading-tight sm:text-5xl">{post.title}</h1>{post.excerpt && <p className="mt-5 text-sm leading-8 text-muted">{post.excerpt}</p>}<div className="mt-5 flex flex-wrap justify-center gap-4 text-[11px] text-muted"><time dateTime={post.publishedAt?.toISOString()} className="flex items-center gap-1.5"><CalendarDays size={14} />{post.publishedAt ? dateFormatter.format(post.publishedAt) : ""}</time>{post.author && <span className="flex items-center gap-1.5"><UserRound size={14} />{post.author}</span>}</div></header>
